...
- MUST detect AI agent User-Agents from the [AI Agent Registry](./ai-bot-detection.md) and serve markdown to them by default
- MUST set `X-AEO-Version` advertising the implemented spec version
- SHOULD set `X-Content-Type-Options: nosniff`
- SHOULD publish `/llms.txt` per the [llms.txt extensions](./llms-txt-extensions.md)
- SHOULD include `.md` URLs in sitemap.xml OR publish `/sitemap.md`

## Test Suite

The reference test runner is [`@dualmark/cli`](../packages/cli):
