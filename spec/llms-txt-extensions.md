...
[`@dualmark/core`](../packages/core/src/llms-txt.ts) ships `renderLlmsTxt(opts)` that produces a conformant `llms.txt` from a structured config:
...
The Astro integration can auto-generate `llms.txt` from collection metadata; see [`@dualmark/astro` README](../packages/astro/README.md).
