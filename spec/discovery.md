...
A conformant server SHOULD publish an `llms.txt` file at the root path (`/llms.txt`) describing the site's AI-relevant content. See [llms-txt-extensions.md](./llms-txt-extensions.md) for AEO-specific extensions to the [llms.txt proposal](https://llmstxt.org).
...
A conformant server SHOULD allow the AI agents listed in the [AI Agent Registry](./ai-bot-detection.md) to crawl the site, unless the operator explicitly intends to block them. Example permissive `robots.txt`:
