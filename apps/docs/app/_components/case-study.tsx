...
export function CaseStudy() {
  return (
    <Section id="case-study">
      <SectionHeader
        eyebrow="Case study · Dodo Payments"
        title={
          <>
            <span className="bg-gradient-to-r from-[#e8ffa8] via-[#c6fe1e] to-[#9ee847] bg-clip-text text-transparent">
              5×
            </span>{" "}
            AI agent visits in two months.
          </>
        }
        description="Dualmark was built and battle-tested at Dodo Payments before it was open-sourced. After rolling it out across the marketing site, AI agent traffic 5×'d in 60 days — without changing a single page of content."
      />

      <div className="relative mx-auto max-w-5xl overflow-x-clip">
        <div
          aria-hidden
          className="pointer-events-none absolute left-1/2 top-1/2 -z-10 size-[320px] -translate-x-1/2 -translate-y-1/2 rounded-full bg-[radial-gradient(ellipse_at_center,rgba(198,254,30,0.10),transparent_65%)] sm:size-[420px] lg:size-[520px]"
        />

        <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
          <StatCard headline="5×" label="AI agent visits" detail="vs. previous 60-day baseline" emphasis />
          <StatCard headline="60d" label="Time to impact" detail="rollout to measurable lift" />
          <StatCard headline="0" label="Content rewrites" detail="same pages, new infrastructure" />
        </div>

        <figure className="mt-8 rounded-[var(--radius-card)] border border-[var(--color-border)] bg-[var(--color-bg-elev-1)]/50 px-6 py-8 md:px-10 md:py-10">
          <svg
            aria-hidden
            viewBox="0 0 24 24"
          ...
          <blockquote className="mt-4 text-pretty text-lg leading-relaxed text-[var(--color-fg)] md:text-xl">
            We treated AI search as an infrastructure problem, not a content
            problem. Dualmark gave every page a markdown twin our existing
            stack could keep shipping. The crawlers found it, started reading
            it, and our agent traffic compounded from there.
          </blockquote>

          <figcaption className="mt-6 flex flex-wrap items-center gap-x-4 gap-y-3 text-sm text-[var(--color-fg-muted)]">
            <DodoLogo className="h-6 w-auto text-[var(--color-fg)]" />
            <span className="hidden h-5 w-px bg-[var(--color-border)] sm:block" />
            <span className="text-sm text-[var(--color-fg-muted)]">
              The Dodo Payments engineering team
            </span>
          </figcaption>
        </figure>

        <div className="mt-8 flex flex-col items-center justify-between gap-4 rounded-[var(--radius-card)] border border-[var(--color-border)] bg-[var(--color-bg)] px-6 py-5 md:flex-row md:gap-6 md:px-8">
          <p className="max-w-md text-pretty text-center text-sm text-[var(--color-fg-muted)] md:text-left">
            Production-ready for early adopters and{" "}
            <Link
              href="https://dodopayments.com"
              className="text-[var(--color-fg)] underline-offset-4 hover:text-[var(--color-accent)] hover:underline"
            >
              live on dodopayments.com
            </Link>{" "}
            today.
          </p>
          <Link
            href="https://dodopayments.com"
            className="group inline-flex h-10 shrink-0 items-center gap-2 rounded-lg border border-[var(--color-border)] bg-[var(--color-bg-elev-1)] px-5 text-sm font-medium text-[var(--color-fg)] transition-colors hover:border-[var(--color-border-strong)] hover:bg-[var(--color-bg-elev-2)]"
          >
            Visit dodopayments.com
...
function StatCard({
  ...
  return (
    <div
      className="relative overflow-hidden rounded-[var(--radius-card)] border bg-[var(--color-bg-elev-1)] p-6"
      style={{
    ...
    >
      {emphasis && (
        <div className="pointer-events-none absolute -right-12 -top-12 size-32 rounded-full bg-[radial-gradient(ellipse_at_center,rgba(198,254,30,0.18),transparent_70%)]" />
      )}
      <div
        className={
          emphasis
            ? "bg-gradient-to-r from-[#e8ffa8] via-[#c6fe1e] to-[#9ee847] bg-clip-text font-mono text-5xl font-medium tracking-tight text-transparent md:text-6xl"
            : "font-mono text-5xl font-medium tracking-tight text-[var(--color-fg)] md:text-6xl"
        }
      >
        {headline}
      </div>
      <div className="mt-3 text-base font-medium text-[var(--color-fg)]">
        {label}
      </div>
...
function DodoLogo({ className, ...props }: SVGProps<SVGSVGElement>) {
  return (
    <svg
      width="192"
      height="30"
